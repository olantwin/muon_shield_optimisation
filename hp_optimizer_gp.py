...
def main():
    ...
    while True:
        try:
            cur.execute(
                '''SELECT params, metric_2, id FROM points_results WHERE metric_2 IS NOT NULL AND tag = 'discrete2_gp' '''
            )
            data = cur.fetchall()
            ...
            #pt_gb = Optimizer(space, GradientBoostingQuantileRegressor(base_estimator=GradientBoostingRegressor(n_estimators=100, max_depth=4, loss='quantile')))
            ...
            for i in range(fraction):
                for j in range(batch_size / fraction):
                    ...
                    cur.execute(
                        '''INSERT INTO points_results (geo_id, params, optimizer, author, resampled, status, tag, min_id) VALUES (%s, %s, %s, 'Artem', 37, 'waiting', 'discrete2_gp', %s) ''',
